...
public:
	...
	Dictmap group;// cost; 
	Dictmapu16 matching, inverseMatching;
	//MOD here
...
public:
	...
	treeNode & operator= (const treeNode &tn) //deep copy
	{
		...
		{			
            ...
            matching = tn.matching;
			inverseMatching = tn.inverseMatching;
...
public:	
	...
	inline int getNumberOfAdjacentverifyGraphEdges(Dictmapu16 m, vector<vector<int > > &adjList, int i)
	{
		int e = 0;
		for (int j = 0; j < adjList[i].size(); j++)
		{
			int idx = adjList[i][j];
			if (mapfind(m,idx,0xff) != UNMAPPED) e += 1;
		}
		return e;
	}
